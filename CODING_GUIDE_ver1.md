...
-- 회원 테이블 인덱스 생성
CREATE INDEX IDX_MEMBER_EMAIL ON MEMBER(MEM_EMAIL);
CREATE INDEX IDX_MEMBER_GOOGLE_EMAIL ON MEMBER(GOOGLE_EMAIL);

-- 회원 테이블 코멘트
COMMENT ON TABLE MEMBER IS '회원 정보 테이블';
COMMENT ON COLUMN MEMBER.MEM_ID IS '회원 ID (PK)';
...
COMMENT ON COLUMN MEMBER.MEM_ADDRESS IS '주소 (구글 캘린더 연동용)';
COMMENT ON COLUMN MEMBER.GOOGLE_EMAIL IS '구글 이메일 (구글 캘린더 연동)';
COMMENT ON COLUMN MEMBER.GOOGLE_CALENDAR_ID IS '구글 캘린더 ID';
...
CREATE TABLE EVENT (
    ...
    UPDATED_AT DATE DEFAULT SYSDATE,            -- 수정일
    CONSTRAINT FK_EVENT_MEMBER FOREIGN KEY (MEM_ID) REFERENCES MEMBER(MEM_ID) ON DELETE CASCADE
);
...
CREATE TABLE TODO (
    ...
    CREATED_AT DATE DEFAULT SYSDATE,            -- 생성일
    UPDATED_AT DATE DEFAULT SYSDATE,            -- 수정일
    CONSTRAINT FK_TODO_MEMBER FOREIGN KEY (MEM_ID) REFERENCES MEMBER(MEM_ID) ON DELETE CASCADE
);
...
```sql
-- 테스트 회원 데이터
INSERT INTO MEMBER (MEM_ID, MEM_PWD, MEM_NAME, MEM_EMAIL, CREATED_AT, UPDATED_AT)
